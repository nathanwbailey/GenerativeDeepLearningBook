...
def CouplingLayer(input_dim, coupling_dim, reg):
...
class RealNVP(models.Model):
    def __init__(self, input_dim, coupling_layers, coupling_dim, regularization):
        ...
        self.masks = tf.convert_to_tensor(np.array([[0, 1], [1, 0]] * (coupling_layers // 2), dtype="float32"))
        self.loss_tracker = metrics.Mean(name="loss")
        self.layers_list = [
            CouplingLayer(input_dim, coupling_dim, regularization) for _ in range(coupling_layers)
        ]
...
model = RealNVP(input_dim=INPUT_DIM, coupling_layers=COUPLING_LAYERS, coupling_dim=COUPLING_DIM, regularization=REGULARIZATION)
...
class ImageGenerator(callbacks.Callback):
    ...
    def generate(self):
        ...
        #Forward pass in non training mode
...
img_generator_callback.display(x, z, samples, save_to='output/before_train.png')
